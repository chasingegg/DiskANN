...
#pragma omp parallel for schedule(dynamic, 1)
  ...
  auto search_numpy_input(
    ...
    py::array_t<unsigned> result_num(1);

    std::vector<unsigned> u32_ids(knn);
    std::vector<_u64>     u64_ids(knn, 2147400000);
    QueryStats            stats;
    ...
    if (l_search == 0) {
      ...
      auto n = result_num.mutable_unchecked<1>();
      n(0) = (unsigned) stats.result_num;
      return std::make_pair(std::make_pair(ids, dists), result_num);
    }
...
#pragma omp parallel for schedule(dynamic, 1)
  ...
  auto batch_search_numpy_input_search_ls(
    ...
    py::array_t<unsigned> ids({num_queries, knn});
    py::array_t<float>    dists({num_queries, knn});
    py::array_t<unsigned> result_nums(num_queries);

    std::vector<_u64>    u64_ids(knn * num_queries, 2147400000);
    diskann::QueryStats *stats = new diskann::QueryStats[num_queries];
...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    auto r = ids.mutable_unchecked();
    auto n = result_nums.mutable_unchecked();
    for (_u64 i = 0; i < num_queries; ++i) {
      for (_u64 j = 0; j < knn; ++j) {
        r(i, j) = (unsigned) u64_ids[i * knn + j];
      }
      n(i) = (unsigned) stats[i].result_num;
    }
    ...
    //   std::cout << stats[i].result_num << std::endl;
    // }
    delete[] stats;
    return std::make_pair(std::make_pair(ids, dists), result_nums);
  }
